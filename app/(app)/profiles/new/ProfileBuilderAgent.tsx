...
const BASICS = ['name','headline','email','phone','location','linkedin','summary'] as const;
...
const keyOf = (prefix: string, id: string | undefined, i: number) => `${prefix}-${id && id.trim() ? id : `idx-${i}`}`;
...
export function ProfileBuilderAgent({ consultantId }: { consultantId?: string }) {
  ...
  return <div className="grid grid-cols-1 xl:grid-cols-[0.95fr_1.35fr] 2xl:grid-cols-[1.05fr_1.55fr] gap-4 xl:gap-5 h-[calc(100vh-140px)] min-h-[620px]">
    ...
    <section ref={profilePaneRef} className="rounded-lg border border-border bg-card h-full min-h-0 overflow-y-auto relative">
      ...
      <Section title="Experience" open={open.experience} highlighted={highlightSection==='experience'} setRef={(el)=>{refs.current.experience = el;}} onToggle={()=>setOpen(p=>({...p, experience:!p.experience}))} onAdd={()=>add('experience')}>{profileState.experience.map((e,i)=><GenericEntry key={keyOf('exp', e.id, i)} entry={e} withSkills highlight={!!highlightPath?.startsWith(`experience.${i}`)} onChange={(patch)=>updateArr('experience', i, patch)} onRemove={()=>remArr('experience', i)} />)}</Section>
      <Section title="Education" open={open.education} highlighted={highlightSection==='education'} setRef={(el)=>{refs.current.education = el;}} onToggle={()=>setOpen(p=>({...p, education:!p.education}))} onAdd={()=>add('education')}>{profileState.education.map((e,i)=><GenericEntry key={keyOf('edu', e.id, i)} entry={e} highlight={!!highlightPath?.startsWith(`education.${i}`)} onChange={(patch)=>updateArr('education', i, patch)} onRemove={()=>remArr('education', i)} />)}</Section>
      <Section title="Skills" open={open.skills} highlighted={highlightSection==='skills'} setRef={(el)=>{refs.current.skills = el;}} onToggle={()=>setOpen(p=>({...p, skills:!p.skills}))} onAdd={()=>add('skills')}><input value={skillsText} onChange={(e)=>updateSkillsText(e.target.value)} className={INPUT} placeholder="Comma-separated quick edit" />{profileState.skills.map((e,i)=><div key={keyOf('skill', e.id, i)} className={`rounded border p-2 grid grid-cols-[1fr_140px_36px] gap-2 ${highlightPath?.startsWith(`skills.${i}`)?'border-primary ring-1 ring-primary/40':'border-border/70'}`}><input value={e.name ?? ''} onChange={(x)=>updateArr('skills', i, { name:x.target.value })} placeholder="Skill" className={INPUT} /><input value={e.level ?? ''} onChange={(x)=>updateArr('skills', i, { level:x.target.value })} placeholder="Level" className={INPUT} /><Button type="button" size="icon-xs" variant="ghost" className="hover:text-destructive" onClick={()=>remArr('skills', i)}><Trash2 size={13} /></Button></div>)}</Section>
      <Section title="Projects" open={open.projects} highlighted={highlightSection==='projects'} setRef={(el)=>{refs.current.projects = el;}} onToggle={()=>setOpen(p=>({...p, projects:!p.projects}))} onAdd={()=>add('projects')}>{profileState.projects.map((e,i)=><GenericEntry key={keyOf('project', e.id, i)} entry={e} withSkills highlight={!!highlightPath?.startsWith(`projects.${i}`)} onChange={(patch)=>updateArr('projects', i, patch)} onRemove={()=>remArr('projects', i)} />)}</Section>
